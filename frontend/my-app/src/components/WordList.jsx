    ...
    <section className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {words.map(({ word, vietnamese, imageUrl, example, synonym }) => {
          // Nếu example là mảng, nối thành chuỗi để truyền vào onSelect nếu cần
          const exampleText = Array.isArray(example) ? example.join('; ') : example;
          // Nếu synonym là mảng, nối thành chuỗi để truyền vào onSelect nếu cần
          ...
          return (
            <button
              key={word}
              onClick={() =>
                onSelect({ word, vietnamese, imageUrl, example: exampleText, synonym: synonymText })
              }
